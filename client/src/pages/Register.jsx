  ...
  return (
    ...
    >
      ...
      <div className="bg-white p-8 max-w-md w-full border border-slate-300 shadow-2xl z-10">
        ...
        <h2 className="text-sm font-headline font-bold text-slate-600 mb-6 text-center uppercase tracking-widest">Register Operator</h2>
        
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[10px] uppercase tracking-widest font-bold text-slate-700 mb-1">Full Name</label>
            <input 
              type="text" 
              required
              value={name}
        ...
        <p className="mt-6 text-center text-slate-600 text-xs font-medium">
          Already an operator? <Link to="/login" className="text-black font-bold hover:underline">Access System</Link>
        </p>
      </div>
    </div>
  );
